...
# `ggplot2`로 시각화하는 이유 {#reason-why-ggplot2}
...
## 시각화를 위한 데이터 전처리 과정 {#ggplot2-survey-preprocessing}

다운로드 받은 데이터에 대한 요약정보를 `summary` 함수를 사용해서 살펴본다.

``` {r ggplot2-surveys-summary}
summary(surveys_dat)
```

### 1 단계 - 결측값 제거 {#surveys-missing}
...
surveys_complete <- surveys_complete %>%
           filter(species_id %in% frequent_species)
```

# 시각화 {#surveys-base-plot-viz}

## Base R 시각화 기능 {#surveys-base-plot}
...
> 
> ```{r number_species_year_with_right_labels_xfont_save, eval=TRUE}
> ggsave("fig/observed_species_in_time.png", width=15, height=10)
> ```
