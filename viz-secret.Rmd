...
그래프를 생성하는데 고생을 하고 있다면, `ggplot2`와 코드를 작성하는 본인에 문제가 있다고 가정하지 마라. 잠시 멈춰서 다음 규칙중 어떤 것이 깨졌는지 곰곰히 생각해보라:

- 데이터프레임에 작업하는 것을 담아라.
- 데이터프레임을 *깔끔하게(tidy)* 만들어라; 자주 데이터 형태를 길게 혹은 폭넓게 변경한다.
- 요인(factor)을 사용하고 요인 자료형에 대해 주인이 된다.
...
```{r viz-secret-gapminder-geom-jitter}
library(ggplot2)
